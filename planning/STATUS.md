...
| `PhxSpace.__add__` mutates the source model during serialization | Bug fix | **Filed — reproduced** — the ERV space-merge runs while writing the file and hands the merged space a *reference* to the first source space's ventilation program, then writes the summed load through it. Exporting one `PhxProject` twice compounds the flows (53.24 → 106.48 → 159.72); WUFI-then-PHPP writes the inflated value into `Addl vent` (PHPP reads the unmerged source spaces). Latent since v1.45.0 (2024-06-05). Shipped CLI/Grasshopper paths are one-export-per-process and unaffected. Fix = fresh `PhxProgramVentilation` with a shared *schedule* (a `deepcopy` would re-introduce the PR #90 dangling-pattern bug) | [`bug-fix/space-merge-mutates-source-graph/`](bug-fix/space-merge-mutates-source-graph/README.md) |

...
| Foundation model shape for PHPP 10.x `Ground` | Feature (cross-repo, honeybee-ph **primary**) | **Scoped** — the honeybee-ph/PHX foundation model is WUFI-shaped; PHPP 10.6 `Ground` asks for seven inputs it lacks ("interior wall towards heated" A/U on slab / unheated basement / crawl space, crawl-space wind shield factor). Field-by-field gap table, required fields with names/defaults, three small model defects, four open decisions for Ed. Blocks `05` Phase 2 and OpenPH's foundation objects | [`features/foundation-phpp10-shape/`](features/foundation-phpp10-shape/README.md) |

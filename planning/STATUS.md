...
| PHPP writer input gaps | Bug fix (5 items) | **Scoped** — the incoming request verified against the code and the blank 10.6 template; three of its five diagnoses changed. `Verification` is a version-guard defect dropping 12 inputs, not a missing `F29` writer; `Ventilation` is two dead cells (`J19` *and* `J20`), not one; `Climate` additionally corrupts `P67`. `SummVent` and `Ground` confirmed as filed. Five phased items, four PRs; both open decisions taken (climate gate = library-codes-when-valid, ground = single-foundation) | [`bug-fix/phpp-writer-input-gaps/`](bug-fix/phpp-writer-input-gaps/README.md) |

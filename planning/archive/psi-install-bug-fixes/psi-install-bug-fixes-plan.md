...
| 5 | METr `lrtb*` arrays filled T,R,B,L | **CONFIRMED — real bug** | An asymmetric HBJSON → PHX → METr save settled the order. Source window C stored top=0.9, right=1.0, bottom=1.1, left=1.2; PHX emitted `[0.9, 1.0, 1.1, 1.2]`; METr displayed those positions as left, right, top, bottom. METr therefore requires L,R,T,B while `to_METr_JSON/metr_schemas.py` wrote T,R,B,L. |
...
**Status: Complete (2026-08-03).** Ed supplied an asymmetric `psi-install-test.hbjson`, its PHX-generated METr project save, and the METr GUI readback. The round-trip directly confirmed that METr interprets `lrtbFrPsi` positions as Left, Right, Top, Bottom. All four shared `lrtb*` window arrays now follow that contract, with a focused asymmetric regression independently asserting width, frame U-value, psi-glazing, and psi-install order. The three existing METr reference cases remain unchanged because their four side values are equal. Verification: focused METr tests (`3 passed`), Black on changed Python files, new-test isort, diff check, and `.venv/bin/python -m pytest tests/` (`789 passed, 3 skipped, 1 deselected`). Full-file isort and Ruff retain unrelated failures already present on `HEAD`: the HVAC `water` import grouping, `SIM108` at line 934, and `F841` at line 1359.

1. Confirm the element order of `lrtbFrW` / `lrtbFrU` / `lrtbGlPsi` / `lrtbFrPsi` from an asymmetric METr export.

...
|---|---|---|---|
| 1 | **[`space-merge-mutates-source-graph`](../../archive/space-merge-mutates-source-graph/README.md)** — `PhxSpace.__add__` mutates the graph during serialization — two exports of one project inflate ventilation flow 53.24 → 106.48 → 159.72 | **Severe, shipping now** | Independent defect, different subsystem, different fix. Reachable from the POC service and any notebook; not from the one-shot CLI. **Filed 2026-08-25.** |

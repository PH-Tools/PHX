...
| D1a | `#115` over-capacity policy: warn and truncate at capacity, matching the existing duct writer (`phpp_app.write_project_vent_ducting`, `PHPPVentDuctWarning` then `rows[:row_capacity]`) | **Proposed**, matches precedent. Writing past the block adds nothing (rows past the block are outside the workbook's `SUMIF`s) and overwrites the rows below it |
| D2 | `04` scope: all phases (1-3) | **Settled** (Ed, 2026-09-12) |
| D3 | `02` on PHPP 9: no change | **Settled by inspection**, see below |
| D4 | `02`: keep the decorative `J19` write, drop `J20` | **Proposed**, as the item doc recommends |
...
2. **`02`**: `wind_protection_class` shape entry with `options` keyed by
   coefficient, `wind_coeff_f` to column `M`, writer + test, **10.x shapes only**
   (D3). Add a test that a 9.x shape writes `N25`/`N26` unchanged and no class.
...
5. **Re-record** with `scripts/perf/record_replay_fixture.py --yes` (Claude-driven;
   ...
   - `Ventilation`: `K19` and `M20` added, `J20` removed, `J19` unchanged
...
6. **Closeout**: `python -m pytest tests/`; tick 01/02/04 on #103; PR body
   ...
   the `02` and `04` closeout steps); file issues for any audit fragility we
   decide to track.

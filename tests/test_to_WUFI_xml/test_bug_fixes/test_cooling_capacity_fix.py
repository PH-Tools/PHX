...
def test_cooling_capacity_fix_300KW_with_2_mech_systems() -> None:
    ...
    assert (
        phx_project.variants[0].mech_collections[0].heat_pump_devices[0].params_cooling.recirculation.capacity == 150.0
    )

    assert len(phx_project.variants[0].mech_collections[1].heat_pump_devices) == 1
    assert (
        phx_project.variants[0].mech_collections[1].heat_pump_devices[0].params_cooling.recirculation.capacity == 150.0
    )

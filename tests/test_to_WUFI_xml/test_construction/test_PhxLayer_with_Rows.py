...
def test_layer_with_two_rows(reset_class_counters) -> None:
    ...
    assert xml_string_to_list(result) == [
        ...
        '<ExchangeDivisionVertical count="2">',
        '<DivisionV index="0">',
...
def test_layer_with_three_rows() -> None:
    # Setup the Base Layer
    layer = PhxLayer()
    mat_1 = PhxMaterial(display_name="mat_1")
    layer.set_material(mat_1)
    ...
    layer.divisions.set_row_heights([1, 2, 3])
    layer.divisions.add_new_column(1)
    mat_2 = PhxMaterial(display_name="mat_2")
    layer.divisions.set_cell_material(0, 1, mat_2)

    mat_3 = PhxMaterial(display_name="mat_3")
    layer.divisions.set_cell_material(0, 2, mat_3)
    ...
    assert xml_string_to_list(result) == [
        ...
        "</Material>",
        '<ExchangeDivisionHorizontal count="0"/>',
        '<ExchangeDivisionVertical count="3">',
        '<DivisionV index="0">',
        "<Distance>1.0</Distance>",
        "<ExpandingContracting>2</ExpandingContracting>",
        "</DivisionV>",
        ...
        "<Type>1</Type>",
        "<IdentNr_Object>4</IdentNr_Object>",
        "</MaterialIDNr>",
        "</ExchangeMaterialIdentNrs>",
    ]

...
def test_vent_patterns_match(
    ...
    for pattern_from_hbjson in utilization_patterns_from_hbjson.values():
        pattern_from_xml = _find_matching_pattern(
            pattern_from_hbjson, utilization_patterns_from_xml.values()
        )
        ...
        assert pattern_from_xml.annual_utilization_days == approx(
            pattern_from_hbjson.annual_utilization_days
        )
        assert pattern_from_xml.relative_utilization_factor == approx(
            pattern_from_hbjson.relative_utilization_factor
        )
        assert pattern_from_xml.annual_operating_hours == approx(
            pattern_from_hbjson.annual_operating_hours
        )
        assert pattern_from_xml.annual_utilization_factor == approx(
            pattern_from_hbjson.annual_utilization_factor
        )

...
from PHX.model.components import PhxComponentOpaque, PhxComponentAperture, PhxApertureElement
...
def _apertures_are_equal(_xml_ap: PhxComponentAperture, _hbjson_ap: PhxComponentAperture) -> bool:
    ...
    if _xml_ap.display_name == _hbjson_ap.display_name:
        ...
        assert abs(_xml_ap.default_monthly_shading_correction_factor - _hbjson_ap.default_monthly_shading_correction_factor) < 0.001

        # -- check the Aperture Elements
        assert len(_xml_ap.elements) == len(_hbjson_ap.elements)
        for xml_ap_element in _xml_ap.elements:
            assert any((_aperture_elements_are_equal(xml_ap_element, hbjson_ap_element) for hbjson_ap_element in _hbjson_ap.elements))

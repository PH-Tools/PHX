...
def test_add_single_column_to_grid(reset_class_counters) -> None:
    ...
    assert grid.cell_count == 0  # No rows = no cells
...
def test_add_single_row_to_grid(reset_class_counters) -> None:
    grid = PhxLayerDivisionGrid()
    grid.add_new_row(1)
    assert grid.cell_count == 0  # No cols = no cells
    assert grid.column_count == 0
...
def test_one_column_exchange_material(reset_class_counters) -> None:
    ...
    assert layer.division_material_id_numbers == [mat_1.id_num, -1]
...
def test_multiple_columns_exchange_material(reset_class_counters) -> None:
    ...
    assert len(layer.exchange_materials) == 1
    assert layer.division_material_id_numbers == [mat_1.id_num, -1, -1]
    ...
    assert layer.division_material_id_numbers == [mat_1.id_num, -1, mat_2.id_num]
...
def test_percentages_single_material_layer():
    layer_1 = PhxLayer()

    assert len(layer_1.materials) == 1
    for mat in layer_1.materials:
        assert mat.percentage_of_assembly == pytest.approx(1.0)


def test_percentage_two_columns_one_material():
    layer_1 = PhxLayer()
...
def test_percentage_two_columns_two_materials():
    layer_1 = PhxLayer()
    mat_1 = PhxMaterial(display_name="mat_1")
    ...
    layer_1.divisions.add_new_row(1)
    mat_2 = PhxMaterial(display_name="mat_2")
    layer_1.divisions.set_cell_material(0, 0, mat_2)

    assert len(layer_1.materials) == 2
    for mat in layer_1.materials:
        assert mat.percentage_of_assembly == pytest.approx(0.5)


def test_single_row_one_material():
    layer_1 = PhxLayer()
    layer_1.divisions.add_new_row(1)
...
def test_two_rows_one_material():
    layer_1 = PhxLayer()
    layer_1.divisions.add_new_row(1)
    layer_1.divisions.add_new_row(1)
...
def test_two_row_two_materials():
    layer_1 = PhxLayer()
    mat_1 = PhxMaterial(display_name="mat_1")
    layer_1.set_material(mat_1)
    layer_1.divisions.set_row_heights([1, 1])
    layer_1.divisions.add_new_column(1)
    mat_2 = PhxMaterial(display_name="mat_2")
    layer_1.divisions.set_cell_material(0, 0, mat_2)
